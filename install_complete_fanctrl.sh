...
install_sudoers() {
    ...
    # Include both platform device path and class device path for compatibility
    local CLASS_HWMON_PATH="/sys/class/hwmon/hwmon$(basename $HWMON_PATH | grep -o '[0-9]*')"
...
# Configure nct6775 module to load automatically
configure_module_loading() {
    print_status "Configuring nct6775 module for automatic loading..."
    
    local modules_file="/etc/modules-load.d/nct6775.conf"
    
    # Check if already configured
    if [ -f "$modules_file" ] && grep -q "nct6775" "$modules_file" 2>/dev/null; then
        print_warning "nct6775 module already configured for automatic loading"
        return 0
    fi
    
    # Add nct6775 to modules-load.d
    echo "nct6775" | sudo tee "$modules_file" > /dev/null
    if [ $? -eq 0 ]; then
        print_success "nct6775 module configured to load automatically on boot"
    else
        print_warning "Failed to configure automatic module loading"
    fi
}
