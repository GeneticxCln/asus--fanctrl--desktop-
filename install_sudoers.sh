...
echo "=== Fan Control Sudoers & Polkit Installation ==="
echo "This script will:"
echo " 1. Install helper to /usr/local/bin/asus-fanctrl-detect"
echo " 2. Install Polkit rules to /etc/polkit-1/rules.d/"
echo " 3. Install Sudoers configuration for user '${TARGET_USER}'"
echo ""
...
# 1. Deploy helper utility
echo "Deploying asus-fanctrl-detect to /usr/local/bin/..."
cp "$SCRIPT_DIR/noctalia-plugin/asus-fanctrl-detect" "/usr/local/bin/asus-fanctrl-detect"
chmod +x "/usr/local/bin/asus-fanctrl-detect"

# 2. Deploy Polkit rules
echo "Deploying Polkit rules..."
cp "$SCRIPT_DIR/noctalia-plugin/50-asus-fanctrl.rules" "/etc/polkit-1/rules.d/50-asus-fanctrl.rules"

# 3. Create the sudoers file with proper permissions
